...
//           const logaccountIdsToDelete = logsToDelete.map(log => log.accountId);
  
//           await db.ActivityLog.destroy({
//             where: {
//               accountId: {
//                 [Op.in]: logaccountIdsToDelete
//               }
//             }
//           });
//           console.log(`Deleted ${logaccountIdsToDelete.length} oldest log(s) for user ${accountId}.`);
//         }
//       }
//     } catch (error) {
//       console.error('Error logging activity:', error);
...
// //           whereClause.accountId = filters.useraccountId;
// //       }
...
// //   }
// // }
// async function getAllActivityLogs({ actionType, startDate, endDate, useraccountId }) {
//   try {
//     const where = {};
//     if (actionType) where.actionType = actionType;
//     if (startDate)   where.timestamp = { [Op.gte]: new Date(startDate) };
//     if (endDate)     where.timestamp = { ...(where.timestamp || {}), [Op.lte]: new Date(endDate) };
//     if (useraccountId)      where.useraccountId = useraccountId;
...
//   const account = await valaccountIdateResetToken({ token });
...
//     await account.save();
// }
// async function valaccountIdateResetToken({token}) { 
...
//     if (!account) throw 'InvalaccountId token';
...
//   if (!refreshToken || !refreshToken.isActive) throw 'InvalaccountId token'; 
//   return refreshToken;
// }
...
//       message = `<p>Please click the below link to reset your password, the link will be valaccountId for 1 day:</p>
...
//       attributes: ['accountId', 'useraccountId','theme', 'notifications', 'language']
//   });
//   if (!preferences) throw new Error('User not found');
//   return preferences;
// }
// async function updatePreferences(accountId, params) {
//   const preferences = await db.Preferences.findOne({ where: { accountId: accountId } });
//   if (!preferences) throw new Error('User not found');

//   // Update only the provaccountIded fields
...
// }
// function basicDetails(account) {
//   const { accountId, title, firstName, lastName, email, phoneNumber, role, created, updated, isVerified, BranchaccountId } = account; 
//   return { accountId, title, firstName, lastName, email, phoneNumber, role, created, updated, isVerified, BranchaccountId };
// }
// async function sendVerificationEmail(account, origin) {
//   let message;
//   if (origin) {
